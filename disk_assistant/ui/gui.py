    ...
    def _start_background_processes(self):
        ...
        if self.speech_recognizer:
            if self.speech_recognizer.initialize():
                ...
                self.listening_stage = "wake_word"  # Start with wake word listening
                self.root.after(500, self._check_voice_input)
        self.root.after(CONFIG["scan_interval"] * 1000, self._periodic_rescan)
    ...
    def _check_voice_input(self):
        if self.speech_recognizer and self.listening_var.get():
            ...
            if text:
                ...
                if self.listening_stage == "wake_word":
                    if "aila" in text:
                        self.status_var.set(
                            "Voice input: 'AILA' detected. Awaiting command..."
                        )
                        self.listening_stage = "command"
                        self.speak("What can I do for you boss?")
                elif self.listening_stage == "command":
                    self.search_var.set(text)
                    self.status_var.set(f"Voice input: '{text}'")
                    self._perform_search()
                    self.listening_stage = (
                        "wake_word"  # Reset to listen for wake word again
                    )
        self.root.after(500, self._check_voice_input)
